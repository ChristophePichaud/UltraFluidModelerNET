...
void CScintillaDemoView::DefineMarker(int marker, Scintilla::MarkerSymbol markerType, COLORREF fore, COLORREF back)
{
  auto& rCtrl{GetCtrl()};
...
void CScintillaDemoView::OnCharAdded(_Inout_ Scintilla::NotificationData* pSCNotification)
{
  ...
  auto pDoc{rCtrl.CreateDocument(10, Scintilla::DocumentOption::Default)};
...
#pragma warning(suppress: 26462)
  ...
  pDoc = rCtrl.CreateDocument(100, Scintilla::DocumentOption::Default);
...
#pragma warning(suppress: 26462)
  ...
  rCtrl.LineIndent();
  rCtrl.LineDedent();
  Scintilla::UndoSelectionHistoryOption udSHO{rCtrl.GetUndoSelectionHistory()}; //NOLINT(clang-analyzer-deadcode.DeadStores, clang-diagnostic-unused-but-set-variable)
  rCtrl.SetUndoSelectionHistory(Scintilla::UndoSelectionHistoryOption::Enabled);
  udSHO = rCtrl.GetUndoSelectionHistory(); //NOLINT(clang-analyzer-deadcode.DeadStores)
  char szSelectionString[100]{}; //NOLINT(modernize-avoid-c-arrays)
#pragma warning(suppress: 26485)
  const Scintilla::Position posSelectionSerialized{rCtrl.GetSelectionSerialized(szSelectionString)};
  UNREFERENCED_PARAMETER(posSelectionSerialized);
#pragma warning(suppress: 26485)
  rCtrl.SetSelectionSerialized(szSelectionString);
  const auto marker{rCtrl.MarkerSymbolDefined(static_cast<int>(Scintilla::MarkerOutline::FolderOpen))};
  UNREFERENCED_PARAMETER(marker);
  int nScale = rCtrl.AutoCGetImageScale();
  rCtrl.AutoCSetImageScale(nScale * 2);
  nScale = rCtrl.AutoCGetImageScale();
  UNREFERENCED_PARAMETER(nScale);
  rCtrl.ScrollVertical(10, 1);
  */
#endif //#ifdef _DEBUG
...
void CScintillaDemoView::OnDwellStart(_Inout_ Scintilla::NotificationData* pSCNotification)
{
  auto& rCtrl{GetCtrl()};
...
void CScintillaDemoView::OnDwellEnd(_Inout_ Scintilla::NotificationData* /*pSCNotification*/)
{
  auto& rCtrl{GetCtrl()};
...
void CScintillaDemoView::OnModifyAttemptRO(_Inout_ Scintilla::NotificationData* /*pSCNotification*/)
{
...
void CScintillaDemoView::OnModified(_Inout_ Scintilla::NotificationData* pSCNotification)
{
...
std::unique_ptr<Scintilla::CScintillaCtrl> CScintillaDemoView::CreateScintillaControl()
{
